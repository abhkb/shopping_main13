...
# YOLO 配置
YOLO_MODEL_PATH = r"D:\OneDrive\Desktop\shopping26\best.pt"
CONF_THRESHOLD = 0.80
...
# GIVE 模式配置
GIVE_COOLDOWN = 2  # 秒
GIVE_ROI_SCALE = 0.6  # ROI 区域大小（相对于画面中心的比例）
...
cap1 = None
cap2 = None
current_camera = 2  # 1: 摄像头1, 2: 摄像头2
...
give_verified = False
give_sent_flag = False  # GIVE模式是否已发送指令的标志
manual_trigger_give = False  # GIVE模式手动触发标志
...
def handle_serial_data():
    ...
    global manual_trigger_need, sent_need_items, give_sent_flag, manual_trigger_give
        ...
        try:
            ...
            if '0' in data:
                ...
                if current_state == STATE_PUT:
                    ...
                    # GIVE模式触发人脸比对
                    manual_trigger_give = True
                    print(f"\n[串口] 🔔 触发GIVE模式人脸比对...")
                    # 重置发送标志，允许重新发送
                    give_sent_flag = False
                else:
                    print(f"\n[串口] ⚠️ 当前状态为 {current_state}，无法触发人脸比对/OCR识别（需要PUT、NEED或GIVE模式）")
            elif '2' in data:
...
def parse_shopping_list(checked_text):
    ...
    prompt = f"""你是一个JSON输出器。只输出JSON数组，不要输出任何其他内容。
                 ...
                 2. 如果文本中提到品牌，必须提取到brand字段，如果没有提到品牌，brand为null，不要凭空添加品牌
...
def can_operate_give():
    """检查 GIVE 模式是否可以操作（冷却时间）"""
    global last_give_operation_time
    current_time = time.time()
    if last_give_operation_time == 0:
        return True
    return (current_time - last_give_operation_time) >= GIVE_COOLDOWN


def reset_give_state():
    """重置 GIVE 模式状态"""
    global give_sent_flag, last_give_operation_time, manual_trigger_give
    give_sent_flag = False
    last_give_operation_time = 0
    manual_trigger_give = False
    print("✅ GIVE 模式状态已重置")


def get_give_roi(frame):
    """获取 GIVE 模式的 ROI 区域（人脸检测区域）"""
    h, w = frame.shape[:2]
    roi_x1 = int(w * (0.5 - GIVE_ROI_SCALE / 2))
    roi_y1 = int(h * (0.5 - GIVE_ROI_SCALE / 2))
    roi_x2 = int(w * (0.5 + GIVE_ROI_SCALE / 2))
    roi_y2 = int(h * (0.5 + GIVE_ROI_SCALE / 2))
    roi = frame[roi_y1:roi_y2, roi_x1:roi_x2]
    return roi, (roi_x1, roi_y1, roi_x2, roi_y2)


def draw_give_roi(frame, roi_coords):
    """绘制 GIVE 模式的 ROI 区域"""
    roi_x1, roi_y1, roi_x2, roi_y2 = roi_coords
    cv2.rectangle(frame, (roi_x1, roi_y1), (roi_x2, roi_y2), (255, 255, 0), 2)
    cv2.putText(frame, "FACE ROI", (roi_x1, roi_y1 - 5),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
    return frame


def detect_faces_in_roi(roi):
    """在 ROI 区域内检测人脸"""
    if roi is None or roi.size == 0:
        return [], []
...
def get_best_face_in_roi(roi):
    """在 ROI 区域内获取面积最大的人脸"""
    face_locations, face_encodings = detect_faces_in_roi(roi)
    ...
    top, right, bottom, left = face_locations[best_index]
    face_encoding = face_encodings[best_index] if face_encodings else None
    face_image = roi[top:bottom, left:right]

    return face_encoding, face_image, (top, right, bottom, left)


def process_give_mode(frame):
    """
    GIVE 模式：人脸比对模式（带 ROI 区域）
    功能：在 ROI 区域内检测人脸，与目标人脸进行比对
    如果是目标顾客，发送 'k' 指令
    """
    global give_sent_flag, last_give_operation_time, current_camera, manual_trigger_give
    
    # 获取 ROI 区域
    roi, roi_coords = get_give_roi(frame)
    
    # 绘制 ROI 区域
    frame = draw_give_roi(frame, roi_coords)
    
    # 显示模式信息
    cv2.putText(frame, "GIVE MODE - Face Verification", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
    
    # 显示当前使用的摄像头
    if current_camera == 1:
        cv2.putText(frame, "Using: Camera 1", (10, 60),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    else:
        cv2.putText(frame, "Using: Camera 2", (10, 60),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    
    # 显示人脸比对状态
    if not has_target:
        cv2.putText(frame, "WARNING: No target face saved!", (10, 90),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        cv2.putText(frame, "Please save target face first (State SAVE)", (10, 110),
    ...
    current_time = time.time()
    if last_give_operation_time > 0:
        remaining = max(0, GIVE_COOLDOWN - (current_time - last_give_operation_time))
        cv2.putText(frame, f"Cooldown: {remaining:.1f}s", (10, 460),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
    
    # 显示提示信息
    cv2.putText(frame, "Place face in ROI and press 'o' to verify", (10, 480),
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
    
    # 检查是否触发识别（按 'o' 键或串口接收 'o'）
    trigger = manual_trigger_give
    
    if trigger:
        manual_trigger_give = False
        
        # 检查是否在冷却期内
        if not can_operate_give():
            remaining = max(0, GIVE_COOLDOWN - (current_time - last_give_operation_time))
            print(f"\n⏰ GIVE 模式冷却中，请等待 {remaining:.1f} 秒")
            cv2.imshow(current_state.upper(), frame)
            return
        
        print("\n" + "=" * 50)
        print("🔍 GIVE模式：正在进行人脸比对...")
        print("=" * 50)
        
        # 在 ROI 区域内检测人脸
        face_encoding, face_image, face_box = get_best_face_in_roi(roi)
        
        if face_encoding is not None and face_box is not None:
            # 进行人脸比对
            is_match, distance = compare_faces(face_encoding)
            
            # 转换 ROI 内坐标到全局坐标
            roi_x1, roi_y1, roi_x2, roi_y2 = roi_coords
            global_top = roi_y1 + face_box[0]
            global_right = roi_x1 + face_box[1]
            global_bottom = roi_y1 + face_box[2]
            global_left = roi_x1 + face_box[3]
            
            if is_match:
                color = (0, 255, 0)  # 绿色 - 匹配
                match_text = f"MATCH! Distance: {distance:.3f}"
                print(f"🎉 目标顾客识别成功！")
                print(f"   人脸距离: {distance:.3f} (阈值: {FACE_DISTANCE_THRESHOLD})")
                print("📤 发送指令: k")
                send_serial_command('k')
                give_sent_flag = True
                last_give_operation_time = time.time()
                
                # 显示匹配信息在画面上
                cv2.rectangle(frame, (global_left, global_top), (global_right, global_bottom), color, 2)
                cv2.putText(frame, match_text, (global_left, global_top - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
            else:
                color = (0, 0, 255)  # 红色 - 不匹配
                match_text = f"NO MATCH - Distance: {distance:.3f}"
                print(f"❌ 人脸不匹配，距离: {distance:.3f}")
                give_sent_flag = False
            ...
            print("❌ 未在 ROI 区域内检测到人脸")
            cv2.putText(frame, "No face detected in ROI", (10, 140),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        
        print("=" * 50 + "\n")
    
    # 实时显示 ROI 内的人脸（不触发比对，仅显示）
    face_encoding, face_image, face_box = get_best_face_in_roi(roi)
    if face_encoding is not None and face_box is not None:
        # 转换坐标到全局
        roi_x1, roi_y1, roi_x2, roi_y2 = roi_coords
        ...
        # 实时绘制人脸框（黄色表示待验证）
        cv2.rectangle(frame, (global_left, global_top), (global_right, global_bottom), (0, 255, 255), 1)
        cv2.putText(frame, "Face detected", (global_left, global_top - 5),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 1)
...
def main():
    ...
    global manual_trigger, current_camera, manual_trigger_need, give_sent_flag, manual_trigger_give
        ...
        while True:
            ...
            if key == ord('q'):
                ...
                if current_state == STATE_PUT:
                    ...
                    print(f"\n⚠️ 当前状态为 {current_state}，无法触发人脸比对/OCR识别（需要PUT、NEED或GIVE模式）")
            elif key == ord('r'):
                reset_tmp_instructions()
                reset_put_keywords()
                reset_need_items()
